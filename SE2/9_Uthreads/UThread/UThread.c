...
VOID UtYield () {
	if (!IsListEmpty(&ReadyQueue)) {
		//running is placed on tail of the list ready to receive cpu when given.
...
VOID UtActivate (HANDLE ThreadHandle) {
	PUTHREAD Th = (PUTHREAD)ThreadHandle;
	InsertTailList(&ReadyQueue, &Th->Link);
	Th->State = Ready;
...
VOID UtJoin(HANDLE threadToWait) {
	...
	if (UtSelf() == threadToWait || !UtAlive(threadToWait))
		return;
	RunningThread->State = Block;
...
//Imidiate context switch to threadToRun if its on ready state.
